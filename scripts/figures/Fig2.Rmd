...
```{r}
# Network stats
data <- read.csv(here::here("output/network_stats/split_year","2007_2016_CA_CrabPorts_2015HAB_NoSubgraph_rmCrab_01contribution.csv"))
colnames(data)
...
# Closure Duration
closure_data <- read.csv(here::here("input_data/DCRB_Historic_Closures_CA_updated.csv"))
...
```{r}
edata <- filter(data, period == "closure") # early season
ldata <- filter(data, period == "open") # late season
```
<br>
