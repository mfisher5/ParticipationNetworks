...
Identify the fisheries and gear types that aren't included in the simplified 3yr plots, for a specific port group and season.
```{r}
p="ERA"
c="early"
g <- readRDS(here::here(outdir, paste0("igraph_",p,"_3yrAVG_",c,".rds")))

l <-layout.circle(g)
l <- cbind(l, 1:vcount(g))
rownames(l) <- V(g)$name
tmpnames <- list(V(g)$name)
# V(g)$colors <- unlist(lapply(tmpnames, function (x) {ifelse(x=="DCRB_POT","darkorange1","dodgerblue4")}))
# png(here::here(pngdir, paste0(p,"_", c,"_3yrAVG_circular_dcrb_labeled.png")),bg="transparent")
