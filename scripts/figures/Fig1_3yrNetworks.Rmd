...
for(p in myports){
  for(c in c("early","late")){
    ...
    for(i in seq(1,length(years))){
      tmpgraph <- readRDS(paste0(indir,"igraph_",p,"_",years[i],"_",c,".rds"))
    ...
    avg_g <- average_participation_networks(multiyr_graph=multiyr_g,annual_graphs=graphs_list,period=c,years=years)
...
This is the plotting function to generate the networks for Figure 1, Fisher et al. 
```{r}
...
for(p in myports){
  ...
  plot_simple(tmpgraph, outdir=pngdir)
}
```
