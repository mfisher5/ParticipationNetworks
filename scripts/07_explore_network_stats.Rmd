...
Check for a correlation between network size (node count) and network statistics, to determine whether network size needs to be incorporated into the generalized linear models (GLMs). Produce Supplementary Figures that show network statistic change through time. 
...
if(!require("facetscales")) {install.packages("facetscales")}
...
```{r}
filename <- "2007_2016_CA_CrabPorts_2015HAB_NoSubgraph_rmCrab_01contribution.csv"
```
<br>

Specify which network statistics to retain from the full data set.
```{r}
